...
btnChart.addEventListener('click', (evt) => {
    ...
    const nameError = validateNames(inputFirstName.value, inputSecondName.value);
    if (nameError !== '') {
        wrongName.innerHTML = nameError; 
        output2.innerHTML = '';
        compatibilityContainer.classList.add('display-none');
        return;  
    }
...
function validateNames(name1, name2) {
    ...
    if (name1.trim() === '') {
        errorMessage += `<p>Partner 1's name cannot be empty.</p>`;
    }

    if (name2.trim() === '') {
        errorMessage += `<p>Partner 2's name cannot be empty.</p>`;
    }
